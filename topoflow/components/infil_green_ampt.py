...
class infil_component( infil_base.infil_component ):
    ...
    #-------------------------------------------------------------------
    _att_map = {
        'model_name':         'TopoFlow_Infiltration_Green_Ampt',
        'version':            '3.1',
        ...
        #-------------------------------------------------------------
        'comp_name':          'InfilGreenAmpt',
        'model_family':       'TopoFlow',
        'cfg_template_file':  'Infil_Green_Ampt.cfg.in',
        'cfg_extension':      '_infil_green_ampt.cfg',
        'cmt_var_prefix':     '/InfilGreenAmpt/Input/Var/',
        'gui_xml_file':       '/home/csdms/cca/topoflow/3.1/src/share/cmt/gui/Infil_Green_Ampt.xml',
        'dialog_title':       'Infiltration: Green-Ampt Parameters',
        'time_units':         'seconds' }
...
##        self.nz_val    = np.zeros([n_layers], dtype='Int16')      #### + nz3
    ...
    def update_infil_rate(self):
        ...
        #-----------------------------------------------------
        # This approach is not totally correct but is stable
        # and gives results similar to the correct method.
        #-----------------------------------------------------
        Green_Ampt_Infil_Rate_v1(self)
...
#-----------------------------------------------------------------------
def Green_Ampt_Infil_Rate_v1(self):
    ...
    #        fc  = infiltrability or "max infiltration rate"
    ...
    #------------------------------------------------------------
    dK = (self.Ks[0] - self.Ki[0])
    dq = (self.qs[0] - self.qi[0])
    fc = (self.G[0] * dK * dq / self.I) + self.Ks[0]
...
#   Green_Ampt_Infil_Rate_v1
#-----------------------------------------------------------------------
def Green_Ampt_Infil_Rate_1D(self, r, r_last, n):
    ...
    if (not(PONDED) and R_GT_KS):    
        ...
        #----------------------------------------------
        #*** if (r_last le Ks) AND (n eq 1) then begin
        if (r_last <= Ks):    
    ...
    #------------------------------------------
        term1 = (t_end - self.tp) / (G * dq)
        term2 = fnp.loat64(1) / (f - Ks)
        term3 = np.float64(1) / (fp - Ks)
        term4 = (f - Ki) / (f - Ks)
        term5 = (fp - Ks) / (fp - Ki)
        term6 = np.log(term4 * term5) / dK
        h     = (term1 - term2 + term3 + term6)   #(eq 0)
        #---------------------------------------------
        dh_df = dK / ((f - Ki) * (f - Ks) ** np.float64(2))
        del_f = -(h / dh_df)
        f = f + del_f
...
#  Green_Ampt_Infil_Rate_1D
#-----------------------------------------------------------------------
def Green_Ampt_Infil_Rate_3D(self, r, r_last, n):
    ...
    if (n_flux > 0):    
        ...
        # Compute test values of Ip  (Smith eqn 5.32)
        #----------------------------------------------
        Ip2 = (G * dq * dK) / (r - Ks)       #(must be 2D arrays  *****)
        Ip1 = (G * dq * dK) / (r_last - Ks)
        dIp = (Ip2 - self.I)
        ...
        #---------------------------------------
        we = np.where(logical_and((dIp < 0), (self.I < Ip1)))
        n_end = size(we[0])
        if (n_end != 0):    
            #-------------------------
            # Note that: Ip = self.I
    ...
    #-------------------------------------------
        term1 = (t_end - tp) / (G * dq)
        term2 = np.float64(1) / (f - Ks)
        term3 = np.float64(1) / (self.fp - Ks)
        term4 = (f - Ki) / (f - Ks)
        term5 = (self.fp - Ks) / (self.fp - Ki)
        term6 = np.log(term4 * term5) / dK
        h = (term1 - term2 + term3 + term6)   #(eq 0)
        #-------------------------------------------------
        dh_df = dK / ((f - self.Ki) * (f - Ks) ** np.float64(2))
        del_f = -(h / dh_df)
        #-------------------------------------------------
        f[wp] = f[wp] + del_f[wp]
        CONVERGED = (absolute(del_f[wp]) <= tol)
        n_tries = n_tries + 1
...
#   Green_Ampt_Infil_Rate_3D
#-----------------------------------------------------------------------
def Green_Ampt_Infil_Rate_v2(self, r, r_last, n):
    ...
    if (SINGLE_PROFILE):    
        Green_Ampt_Infil_Rate_1D(self, r, r_last, n)
    else:    
        Green_Ampt_Infil_Rate_3D(self, r, r_last, n)
